...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const registerMember = async (memberData: any): Promise<{ success: boolean; member?: Member; error?: string; needsEmailVerification?: boolean; churchName?: string }> => {
    try {
      const result = await DatabaseService.createMember(memberData);
      
      if (result) {
        await saveMemberToStorage(result.member);
        return { 
          success: true, 
